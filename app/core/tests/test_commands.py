...
class CommandTests(TestCase):
    ...
    # @patch removes the 1 min delay if an operational error is returned to speed up the test
    @patch("time.sleep", return_value=True)
    def test_wait_for_db(self, ts):
        """Test waiting for db"""
        with patch("django.db.utils.ConnectionHandler.__getitem__") as gi:
            # tries to connect to the DB 5 times which also raises the OperationalError 5 times
